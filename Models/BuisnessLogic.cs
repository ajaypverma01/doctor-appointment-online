...
namespace DoctorPatient.Models
{
    public class BuisnessLogic
    {
        ...
        public static EmailBody GetAppointmentDetailsById(AppointmentModel appointment, EmailType emailType)
        {
            HospitalDbContext db = new HospitalDbContext();
            var appointmentDetails = (from a in db.Appointments
                                      join d in db.Doctors on a.DoctorID equals d.ID
                                      join au in db.ApplicationUsers on a.UserID equals au.Id
                                      ...
                                      select new
                                      {
                                          ...
                                          TimeBlock = a.TimeBlockHelper,
                                          EmailTo = au.Email
                                      }).FirstOrDefault();

            EmailBody body = new EmailBody();
            body.PatientName = appointmentDetails.PatientName;
            body.DoctorName = appointmentDetails.DoctorName;
            body.AppointmentDate = appointmentDetails.AppointmentDate.ToShortDateString();
            body.AppointmentTime = appointmentDetails.TimeBlock;
            body.ClinicName = "HCL Healthcare";
            body.ClinicPhoneNumber = "9711305745";
            body.ClinicAddress = appointmentDetails.ClinicAddress;
            body.AppointmentID = appointmentDetails.AppointmentID.ToString();
            body.EmailTo = appointmentDetails.EmailTo;
